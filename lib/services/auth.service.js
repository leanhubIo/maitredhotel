 ...
 */
const removeToken = module.exports.removeToken = function (userId) {
...
const findByToken = module.exports.findByToken = function (candidateToken) {
    ...
    return User.find({ token: candidateToken }).exec()
        .then((users) => {

            if (users.length > 1){
                const removeAllTokens = users.map((user) => removeToken(user._id));
                return Promise.all(removeAllTokens)
                    .then(() => Promise.reject(Boom.conflict('similar.tokens')));
            }
